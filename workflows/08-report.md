...
- Under `tools/enforced_audit_driver.py`, `tools/publish_bilingual_reports.py` is invoked with `--out audit-output` so it writes bilingual report trees beneath the audit root while preserving final summary artifacts under `audit-output/06-report`.
- Final summary reports and final machine summary JSON are generated by `tools/generate_final_report.py --out audit-output/06-report`.
- PoC artifacts belong to validation output: `audit-output/04-validation/poc-tests`. Do not use `audit-output/machine/poc-tests` as the report-stage PoC root.

## Required steps (in order)

1. **Generate PoC artifacts** for each reportable finding (if not already done in 06-validation):
   `tools/generate_poc_testcase.py --findings <finding-index.json> --generate-from-finding --languages <lang1,lang2,...> --profile <package-profile.json> --out audit-output/04-validation/poc-tests`
...
2. **Publish bilingual reports** (machine/zh-CN/en-US):
   ...
   - Enforced driver runs use `--out audit-output --skip-final-report`, producing the same bilingual report tree under the audit root and leaving final summary generation to step 6.
...
4. **Validate report completeness** (checks discovery_method, poc_test_artifacts, disclosure_status):
   `tools/validate_report_completeness.py --findings <finding-index.json> --correlation <correlation.json> --report-root audit-output --manual-root audit-output/04-validation/manual-review --poc-root audit-output/04-validation/poc-tests`

5. **Validate PoC artifacts**:
   `tools/validate_poc_artifacts.py --poc-root audit-output/04-validation/poc-tests`
...
- audit-output/04-validation/poc-tests/*/ (PoC artifact directories)
