...
def main():
    with tempfile.TemporaryDirectory() as td:
        ...
        findings.write_text(json.dumps({'findings':[
            {'id':'FINDING-001','status':'Validated','title':'readelf relocation crash','summary':'missing bounds check relocation sh_link','affected_component':{'package':'binutils','component':'readelf','version_or_commit':'2.44'},'source_code_evidence':[{'file':'binutils/readelf.c','function':'display_relocations'}],'root_cause':'missing bounds check relocation sh_link','security_impact':'crash out-of-bounds read','validation':{},'cvss':{},'fix_recommendation':'add bounds check','disclosure_level':'D3-maintainer-private'},
            {'id':'FINDING-002','status':'Validated','title':'different','summary':'different parser issue','affected_component':{'package':'binutils','component':'objdump'},'source_code_evidence':[{'file':'binutils/objdump.c','function':'x'}],'root_cause':'different issue','validation':{},'cvss':{},'fix_recommendation':'fix','disclosure_level':'D3-maintainer-private'}]}))
