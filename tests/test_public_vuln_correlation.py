...
def main():
    with tempfile.TemporaryDirectory() as td:
        ...
        findings.write_text(json.dumps({'findings':[
            ...
            {'id':'FINDING-003','status':'Validated','title':'unrelated package issue','summary':'issue tracked as CVE-2026-0001 in openEuler registry','affected_component':{'package':'unrelated-pkg','component':'core'},'source_code_evidence':[{'file':'src/core.c','function':'init'}],'source_to_sink_path':'input -> init -> issue','root_cause':'registry-only match fixture','security_impact':'low impact','validation':{},'cvss':{},'fix_recommendation':'fix','disclosure_level':'D2-internal-validated','discovery_method':[{'type':'manual','description':'fixture'}],'disclosure_status':'unknown'},
        ]}))
        ...
        oe_index=t/'cve-index.json'
        oe_index.write_text(json.dumps({
            'schema_version': '1.0',
            'record_count': 1,
            'index': {
                'CVE-2026-0001': [{
        ...
        oe003=byid['FINDING-003']
        assert oe003['status']=='publicly_disclosed'
        assert oe003['match_level']=='M3'
        assert oe003['matched_records'][0]['source']=='openEuler-Registry'
        assert oe003['matched_records'][0]['id']=='CVE-2026-0001'
